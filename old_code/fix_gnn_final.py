# fix_gnn_final.py
# Final fix for GNN target creation issue
...
def build_daily_graphs_final(df_all, adj_matrix, actual_cities):
    """Build daily graphs with proper target handling."""
    print("Building daily graphs (final version)...")
...
def main():
    ...
    try:
        ...
        graphs, targets, ghi_scaler, target_scaler, graph_dates, graph_cities = build_daily_graphs_final(df_all, adj_matrix, actual_cities)
        ...
        print(f"\nFinal fix completed in {total_time:.1f} seconds!")

    except Exception as e:
        print(f"ERROR: Final fix failed with exception: {e}")
        import traceback
        traceback.print_exc()
        return
