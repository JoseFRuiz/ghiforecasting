# fix_gnn_targets.py
# Focused fix for GNN target creation issue
...
def build_daily_graphs_fixed(df_all, adj_matrix, actual_cities):
    ...
    max_graphs = 100  # Increased for better training
    ...
    for i, date in enumerate(unique_dates):
        ...
        if len(node_features) > 0:
            ...
            if i < 5:
                ...
                print(f"      Original mean: {target_scaler.inverse_transform(np.array([[y]]))[0, 0]:.4f}")
...
def main():
    ...
    try:
        ...
        graphs, targets, ghi_scaler, target_scaler, graph_dates, graph_cities = build_daily_graphs_fixed(df_all, adj_matrix, actual_cities)
        ...
        print(f"\nFix completed in {total_time:.1f} seconds!")

    except Exception as e:
        print(f"ERROR: Fix failed with exception: {e}")
        import traceback
        traceback.print_exc()
        return
